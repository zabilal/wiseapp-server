...
//    private JsonArray roomList = new JsonArray();
    private Map<String, List<Message>> offlineMessage = new HashMap<>();
...
//                sendBus(client.getId(), Serializer.pack(new Room(Configuration.SERVER_ROOM, Configuration.SERVER_TOPIC).setSystem(true)));
     ...
     * @param client
     *
     * This guys checks the server cache to see if the newly connected client has offline messages,
     * if offline messages exist for client then he delivers the messages in a shaprapra manner
     * like a delivery man.
     */
    private void pushOfflineMessages(ClientID client) {
        if (offlineMessage.containsKey(client.getPhoneNumber())){
            List<Message> offlines = offlineMessage.get(client.getPhoneNumber());
            for (Message message : offlines){
                sendBus(client.getId(), message);
            }
            //After messages are delivered, delete them
            offlineMessage.remove(client.getPhoneNumber());
        }
    }
    ...
    protected void joinRoom(ClientID client, Room room) {
        ...
        if (rooms.containsKey(name)) {//if this room is already existing
//            sendBus(client.getId(), Serializer.pack(new Room(rooms.get(name).getSettings(), room.getCreated())));  //Original
        ...
        } else {//if the room is not yet existing
            sendBus(client.getId(), new Room(name, room.getTopic(), client.getPhoneNumber(), client.getId()));  //Original
...
//            client.setRoom(null);
     ...
     * if a member in the room is offline the message is cached
     * for later delivery in shaprapra manner.
     *
...
//                    sendBus(client.getId(), message);
                    messageClient(client.getPhoneNumber(), message);
            }
        }
    }
...
//                    sendBus(client.getId(), message);
     ...
     * if the user is not online the message is saved
     * for later delivery
     *
    ...
    protected void messageClient(String receiver, Object message) {
        if (clients.get(receiver) != null) { //checking to see if client is online, then send message
        ...
        }else { //client is offline then save to offline messages
            if (offlineMessage.containsKey(receiver)){
                offlineMessage.get(receiver).add((Message) message);
            }else{
                List<Message> offline = new ArrayList<>();
                offline.add((Message)message);
                offlineMessage.put(receiver, offline);
            }
        }
    }
